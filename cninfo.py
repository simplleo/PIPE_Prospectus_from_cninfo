...
def cleanFormat(data):
    ...
    data=re.sub('<br>([第（]?[一二三四五六七八九十0-9]+(([节章）]、?)|、).{1,32}?)(<br>)+'+\
                ...
                '\n\n\\1\\5\n<br>',data)
    #-----：后面的br改成\n--
    data=re.sub('<br>([^，。1-9a-zA-Z]{1,10}?：.*?)<br>','<br>\\1\n',data)
    #-----。后面的br改成\n------
    data=re.sub('([。;:！？])<br>','\\1\n',data)
    #去掉剩余的<br>     
    data=re.sub('<br>','',data)
...
def get_anmt_summary(pageURL,anmtType):   
    fulltext=get_anmtHX_fulltext(pageURL)
    summary=None
##    p=fulltext.find('公司英文名称')
##    if p>=100:
##        print(fulltext[p-100:p+2000])
...
if __name__ =="__main__":
    ...
    if test==0:
        ...
        #调试时使用
        anmttest={
            'announcementTime': 1472400000000, 
            'announcementTitle': '2016年度非公开发行A股股票预案（第二次修订稿）',
            'secCode': '002678' }
        report_url=get_HX_anmt_url(anmttest)
        if report_url:
            print(report_url)
            anmt_summary=get_anmt_summary(report_url,2)
            #anmt_summary=get_anmt_content(report_url)
            if anmt_summary:
                pass
                print(anmt_summary)
